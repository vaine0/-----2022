...
const float Cos[8] = {1.000, 0.707, 0.000, -0.707, -1.000, -0.707, 0.000, 0.707};
const float Sin[8] = {0.000, 0.707, 1.000, 0.707, 0.000, -0.707, -1.000, -0.707};
const unsigned int  ref_time  = 1000 / 100;      // THD刷新时间间隔; n ms / 100 (防止溢出)
const float ref_vcc = 3300.0; // ad参考电压
// 采样点数
#define sample_num 8
const unsigned int  samp_freq = sample_num * 1000; // 波形采样频率; Hz

float THD = 0.000; // 全局变量THD, display()始终显示THD; 计算函数refresh_THD()在计时中断进入, 不断更新算得的THD
